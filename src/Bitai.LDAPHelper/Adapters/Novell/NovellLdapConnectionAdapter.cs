...
namespace Bitai.LDAPHelper.Adapters.Novell
{
    ...
    public class NovellLdapConnectionAdapter : ILdapConnectionAdapter
    {
        ...
        public async Task<ILdapSearchQueueAdapter> SearchAsync(string searchBase, int searchScope, string searchFilter, string[] attributeNames, bool typesOnly, ILdapSearchConstraintsAdapter constraints) {
            var novellConstraints = constraints != null ? ((NovellLdapSearchConstraintsAdapter)constraints).GetWrappedConstraints() : null;

            LdapSearchQueue searchQueue = await _ldapConnection.SearchAsync(searchBase, searchScope, searchFilter, attributeNames, typesOnly, (LdapSearchQueue)null, novellConstraints);
