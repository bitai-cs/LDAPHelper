...
namespace Bitai.LDAPHelper
{
	public class AccountManager : BaseHelper
	{
		...
		/// <returns>A Task of <see cref="LDAPAddUserAccountResult"/></returns>
		public async Task<LDAPAddUserAccountResult> CreateUserAccountForMsAD(LDAPMsADUserAccount newUserAccount, string distinguishedNameOfContainer, string requestLabel = null)
		{
			try
			{
				#region Validate minimally required properties
				if (string.IsNullOrEmpty(newUserAccount.Cn))
					throw new InvalidOperationException($"{nameof(newUserAccount.Cn)} attribute is required.");

				if (string.IsNullOrEmpty(newUserAccount.DisplayName))
					throw new InvalidOperationException($"{nameof(newUserAccount.DisplayName)} attribute is required.");

				if (string.IsNullOrEmpty(newUserAccount.SAMAccountName))
					throw new InvalidOperationException($"{nameof(newUserAccount.SAMAccountName)} attribute is required.");

				//// DistinguishedName will be autogenerated lines below.
				//if (string.IsNullOrEmpty(newUserAccount.DistinguishedName))
				//throw new InvalidOperationException($"{nameof(newUserAccount.DistinguishedName)} attribute is required.");

				if (newUserAccount.ObjectClass == null || newUserAccount.ObjectClass.Length == 0)
					throw new InvalidOperationException($"{nameof(newUserAccount.ObjectClass)} attribute is required.");
				#endregion

				//Generate user account DistinguishedName LDAP attribute
				newUserAccount.DistinguishedName = $"CN={newUserAccount.Cn},{distinguishedNameOfContainer}";
				...
				return new LDAPAddUserAccountResult(requestLabel)
				{
			...
			{
				return new LDAPAddUserAccountResult("Error creating user account.", ex, requestLabel);
			}
		}
